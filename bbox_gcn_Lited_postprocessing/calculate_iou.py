...
def generate_mask(new_image, last_dict):
    ...
    for key in dict_pred_mask.keys():
        dict_pred_mask[key] = np.zeros((260,260))
...
def generate_one_dim_mask(dict_pred_mask):
    ...
    rooms = {'84, 139, 84':1,'255, 255, 255':2, '0, 0, 0':3, '255, 0, 255':4, '115, 198, 205':5, '139, 26, 85':6, '130, 134, 139':7, '128, 0, 0':8, '255, 144, 30':9, '58, 58, 139':10, '235, 206, 135':11, '42, 42, 165':12, '0, 100, 0':13}
    rooms_ = [np.array(room) for room in rooms]
    one_dim_pred_mask = np.zeros((260,260))
    for room in rooms:
